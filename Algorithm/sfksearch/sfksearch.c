...
static int KTrieInsert( KTRIE_STRUCT *ts, KTRIEPATTERN * px  )
{
   ...
   */
   ...
   if( root->pkeyword )
   {
      /*一个节点有可能是多个关键字重复,只是关键字的Id不同,但是内容相同*/
...
static inline int KTriePrefixMatch( KTRIE_STRUCT  * kt, 
                                    ...
                                    unsigned char * bT, 
                                    int n,
       int(*match)( int id,  int index, void * data ),
       void * data )
{
   ...
   while( n )
   {
     if( root->edge == *T )
     {
         ...
         printf("start匹配节点有可能存在n个id不同,但key相同的情况\n");
         //if(n == 0) 字符串以某一个key结束时候才产生命中结果.也就是说完全匹配或者以某一个key结束
